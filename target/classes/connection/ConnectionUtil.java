...
public class ConnectionUtil {
    ...
    public static List<String> Connect(String address){
        ...
        HttpURLConnection conn1 = null;         //属性加1的是用来获取动态ip的
        URL url = null;
        InputStream in = null;
        ...
        while (true){       //动态IP不稳定，经常超时抛异常，所以正常就会break，异常就continue
            num ++;
            try {
               if (num%10 == 0){
                    URL url1 =new URL("http://dynamic.goubanjia.com/dynamic/get/");    //爬虫API
                    url = new URL(address);
                ...
                conn = (HttpURLConnection) url.openConnection(proxy);      //通过动态代理访问
